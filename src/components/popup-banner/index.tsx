...
export default function Modal({ app }: Props) {
  ...
  const handleClose = () => {
    // Pindah ke iklan berikutnya hanya saat tombol close ditekan
    setCurrentIndex((prevIndex) => prevIndex + 1);
  };

  // Periksa apakah ada iklan yang tersisa
  if (bannerImage.data && currentIndex < bannerImage.data.length) {
    const data = bannerImage.data[currentIndex];
                ...
                <div className="mb-6">
                  <div className="relative">
                    <div className="mx-auto w-full">
                      ...
                      <button
                        className="absolute right-0 top-0 m-2 rounded bg-transparent p-2 font-semibold leading-none text-black outline-none hover:bg-red-600 focus:outline-none animate-blink"
                        onClick={handleClose}
                      >
                        <svg
                          className="w-6"
    ...
    // Ketika septofmua iklan sudah selesai atau tidak ada iklan lagi
    return null;
  }
}
