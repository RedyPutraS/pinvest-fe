...
export interface PiLearningSliderProps {
...
export default function PiLearningSlider(props: PiLearningSliderProps) {
  const { items, rounded } = props;
  return (
    <div className="flex justify-center">
      <div className="mx-2 w-screen  xl:max-w-[1400px]">
        <Carousel
          interval={3000}
          autoPlay
          infiniteLoop
          ...
          renderIndicator={(onClickHandler, isSelected, index, label) => {
            if (isSelected) {
              return (
                <li
                  className="mx-1.5 -mb-2 inline-block h-0.5 w-6 rounded-full bg-pv-cyan md:mx-2 md:mb-0  md:h-1  md:w-16"
            ...
            return (
              <li
                className="mx-1.5 -mb-2 inline-block h-0.5 w-6 rounded-full bg-pv-grey-light3 md:mx-2 md:mb-0 md:h-1  md:w-16"
                value={index}
                onClick={onClickHandler}
            ...
            return (
              ...
              >
                <img
                  ...
                  className={`h-full w-full object-contain xl:h-[350px] md:object-fill md:h-[260px] xl:object-cover ${
                    rounded && `rounded-lg`
                  } `}
                ...
                {item.id && (
                  <div className="absolute top-0 h-2/3 w-full bg-transparent"></div>
                )}
                {item.id && (
                  <div className="absolute bottom-0 h-full w-1/3 bg-transparent"></div>
                )}
                {item.id && (
                  <div className="absolute bottom-0 right-0 h-full w-1/3 bg-transparent"></div>
                )}
              </div>
            );
          })}
